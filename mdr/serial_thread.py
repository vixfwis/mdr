...
from mdr.const import MODE_WRITE, MODE_READ, ACK, ABORT
...
class SerialThread(threading.Thread):
    ...
    def run(self):
        ...
        try:
            ...
            if self.serial_port.read(1) == ACK:
                self.connected = True
        ...
        if self.connected:
            ...
            while not self._stop_event.is_set():
                ...
                if self._abort_event.is_set():
                    self.serial_port.write(ABORT)
                    self.serial_port.read(1)
                    self.set_write()
                    self.response_factory.reset()
                    self._abort_event.clear()
                if self.mode == MODE_WRITE and not self.requests.empty():
                    ...
                    if self._message.expect_response() is not None:
                        ...
                        self.set_read()
                elif self.mode == MODE_READ and self.serial_port.in_waiting > 0:
                    t = time()
